# CLAUDE.md — 通用 AI 编码助手行为规范

> 以下规范用于减少常见 LLM 编码错误。可与项目特定指令按需合并。
>
> **权衡：** 这些指南倾向于谨慎而非速度。对于琐碎任务，请自行判断。
...
- **核心模型**：
  ...
  - `InterviewEvaluation`（结构化面试评估；关联 `Interview` 与面试官 `User`，`dimensions` Json 存维度评分 `[{name,score,comment?}]`、`overallScore`、`conclusion`、`submittedAt`（null=待填写）、`remindedAt`（催收标记）；评估维度由字典 `evaluation_dimension` 配置）
  - `OperationLog`（操作日志）
...
- 阶段 5 切片进度（PRD 见根目录 PRD_阶段5 文档）：S0 工具链 ✅ / F2-S 简历打分服务端 ✅（迁移 `20260901000000_add_ai_match_score` 已生成待人工 apply）/ F2-C 前端 ✅ / F1-S ✅ / F1-C ✅ / F3-S ✅（迁移 `20260902000000_add_interview_question_outline` 已生成待人工 apply）/ F3-C ✅ / F5-S ✅（迁移 `20260902130000_add_agency_referral` 已生成待人工 apply）/ F5-C ⬜ / F4-S1 ⬜ / F4-S2 ⬜ / F4-C ⬜ / M6 ⬜
